...
export function CookieConsentPrivacyFooter() {
  const consent = useSyncExternalStore(subscribeToConsent, getConsentFromCookie, getServerConsent)
  ...
  return (
    <footer className='border-border flex flex-col gap-2 border-t pt-6 sm:flex-row sm:items-center sm:justify-between'>
      <p className={cn('text-xs sm:text-sm', isGranted ? 'text-muted-foreground' : 'text-foreground')}>
        {isGranted
